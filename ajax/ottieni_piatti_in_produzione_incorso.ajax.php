<?php
    ...
    if($date <=0){
		...
		$idprg=0;
		$idprg_next=0;
		$query_idprog="SELECT min(idprogrammazione) as idprg FROM programmazioneordini WHERE stato = 2 AND categoria IN ('primo','secondo','contorno') AND serata='$date'";
	    ...
	    if($idprg>0){
			...
			AND stato=2
			AND categoria IN ('primo','secondo','contorno')
			AND serata='$date'
